...
export default function CleanerCard({ cleaner, onBook }) {
  const providerLabel = cleaner.provider_type === 'company' ? 'Company' : 'Individual'
  return (
      ...
      <div className="p-4">
        <div className="flex items-start justify-between">
          ...
          <div className="text-right">
            <span className={`block text-xs px-2 py-1 rounded-full ${cleaner.is_available ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
              {cleaner.is_available ? 'Available' : 'Offline'}
            </span>
            <span className="mt-1 inline-block text-xs px-2 py-1 rounded-full bg-blue-50 text-blue-600 border border-blue-100">
              {providerLabel}
            </span>
          </div>
        </div>
        ...
        <div className="mt-3 text-sm text-gray-600 flex items-center gap-2">
          ...
          <span>
            {cleaner?.location?.address || 'Nearby'}
            {typeof cleaner.distance_km === 'number' && (
              <span className="text-gray-500"> • {cleaner.distance_km} km</span>
            )}
          </span>
        </div>
